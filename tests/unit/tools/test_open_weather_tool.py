...
@pytest.fixture()
def client():
    return OpenWeatherTool(api_key="YOUR_API_KEY")
...
def mock_requests_get(*args, **kwargs):
    if args[0] == OpenWeatherTool.GEOCODING_URL:
        return MockResponse([{"lat": 40.7128, "lon": -74.0061}], 200)
    elif args[0] == OpenWeatherTool.BASE_URL:
...
def test_invalid_location(mock_get, client):
    ...
    assert isinstance(result, ErrorArtifact)
    assert (
        "Error fetching coordinates for location: InvalidCity, XX" in result.to_text()
    )
